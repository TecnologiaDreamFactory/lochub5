...
function initSolucoesMosaic() {
    ...
    function mosaicWebpPath(src) {
        var m = src.match(/^img\/(.+)\.(png|jpg|jpeg)$/i);
        if (!m) return src.replace(/\.(png|jpg|jpeg)$/i, '.webp');
        return 'img/webp/solucoes/' + m[1] + '.webp';
    }
    ...
    function openZoom(item) {
        ...
        zoomed.decoding = 'async';
        zoomed.src = mosaicWebpPath(item.src);
        zoomed.alt = item.alt || '';
        zoomed.onerror = function () {
            if (zoomed.src.indexOf('/webp/solucoes/') !== -1) {
                zoomed.onerror = null;
                zoomed.src = item.src;
            }
        };
    ...
    photos.forEach(function (item, i) {
        ...
        var webp = mosaicWebpPath(item.src);
        ...
        img.alt = item.alt;
        img.decoding = 'async';
        img.sizes = '(max-width: 900px) 23vw, 22vw';
        if (i < 4) img.fetchPriority = 'high';
        else if (i > 8) img.fetchPriority = 'low';
